            ...
            handler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    if (mBackStack.size() >= 2) {
                        List<Fragment> fragments = mFragmentManager.getFragments();
                        Fragment activeFragment = fragments.get(fragments.size() - 2);
                        activeFragment.onPause();
                        activeFragment.onStop();
                    }
                }
            }, 300);
        }
//    ft.replace(mContainerId, frag);
        ...
        if (frag.isAdded()) { // if the fragment is already in container
            ft.show(frag);
...
//      this(navigatorProvider.getNavigator(IpmFragmentNavigator.class));
//    }
//
//    /**
//     * Construct a new fragment destination. This destination is not valid until you set the
